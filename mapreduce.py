# Map-Reduce python script, uses Spark-Core API to optimise time-intensive tasks 
# (parsing and using map-reduce framework to get national average, state-wise average, ratio) 
# uses tabulate to format results table
...
# parse required fields (get rid of illegal values)
def parser(line):
	fields = line.split(",")
	if(fields[1].isdigit()):
...
formatted_results = [(state, avg, ratio) for state, (avg, ratio) in data]
